...
function addAndRunScript() {
  ...
  file=$2
  eval args="${3:-false}"
  /root/.sdkman/candidates/groovy/current/bin/groovy -Dgroovy.grape.report.downloads=true resources/conf/addUpdatescript.groovy -u "$username" -p "$password" -n "$name" -f "$file" -h "$nexus_host"
...
#nexus_maven defaults
  blob_store=default
  strict_content_validation=true
  version_policy=release 
          # release, snapshot or mixed
...
#nexus_maven group
  ...
  member_repos=(maven-central aem)
...
if [ "${AEM_ENABLED}" = "true" ]
  ...
  echo "$(date) - Adding AEM and Maven Central repos to a new AEM group ..."
  addAndRunScript aemGroup resources/conf/create_repo_maven_group_AEM.groovy "\${GROUP_CONFIG}"
fi

echo "$(date) - Adding a new task that cleans old snapshots ..."
addAndRunScript removeSnapshotsTask resources/conf/create_task_removeSnapshots.groovy
